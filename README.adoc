...
OpenRewrite has a lot of individual recipes for this, but we can also use `org.openrewrite.recipe:rewrite-migrate-java:2.1.0` => `org.openrewrite.java.migrate.guava.NoGuava` which has no required input.
