...
Both for Maven and Gradle, we can run the migrations either by modifying our build files or by running a shell command or an init script, respectively.
...
These replacements, which offer compiler and type support, allow us to do clear before & after replacements, with the caveat that they're whitespace agnostic.
We could for example use them to facilitate replacing `StringUtils.equals(..)` with `Objects.equals(..)`.

A Refaster template recipe is an imperative recipe that's generated automatically when we build our Java classes that contain one or more Refaster templates.
...
A couple of things to keep in mind regarding a Refaster recipe:
...
For now, we've used 2 quite basic recipes, which had a relatively limited impact.
...
OpenRewrite has a lot of individual recipes for this, but we can also use `org.openrewrite.recipe:rewrite-testing-frameworks:3.27.0` => `org.openrewrite.java.testing.hamcrest.MigrateHamcrestToAssertJ`, which has no required input.
...
One of the recipes we can use for this is https://docs.openrewrite.org/recipes/java/testing/junit5/junit4to5migration[org.openrewrite.java.testing.junit5.JUnit4to5Migration] for which we'll need a dependency on `org.openrewrite.recipe:rewrite-testing-frameworks:3.27.0`.
...
OpenRewrite has a lot of individual recipes for this, but we can also use `org.openrewrite.recipe:rewrite-migrate-java:3.27.1` => `org.openrewrite.java.migrate.guava.NoGuava`, which has no required input.
...
We can run a lot of recipes manually, such as `org.openrewrite.staticanalysis.MissingOverrideAnnotation`, but our eye swiftly gets drawn to https://docs.openrewrite.org/recipes/staticanalysis/commonstaticanalysis[org.openrewrite.staticanalysis.CommonStaticAnalysis] which is part of `org.openrewrite.recipe:rewrite-static-analysis:2.27.0` and has no required input.
...
There are quite a few https://docs.openrewrite.org/recipes/github[recipes] to help you manage your GitHub workflows.
...
Which makes it a lot easier for us to find these kinds of issues.
